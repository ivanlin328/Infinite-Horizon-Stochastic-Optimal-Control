...
def is_collision(robot_pos, obstacles, robot_radius):
    """
    Checks if the robot collides with any of the circular obstacles.

    Args:
        robot_pos (np.ndarray): The robot's current 2D position (x, y).
        obstacles (np.ndarray): An array where each row is an obstacle defined
                                 as [center_x, center_y, radius].
